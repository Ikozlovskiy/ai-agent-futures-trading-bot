...
def _fit_line(swings: List[Tuple[int, float]]) -> Optional[Tuple[float, float]]:
    """
    Fit y = a*x + b through swings using simple least-squares on integer index.
...
def _safe_arrays(snap: Dict, keys: List[str]) -> bool:
    """Ensure snapshot has arrays and enough history (>= 5 bars)."""
...
def _get_atr_from_itf(itf_setup: Optional[Dict]) -> float:
    """Read ATR from itf_setup if present, else 0.0 (caller decides to return None)."""
    try:
...
# =========================
# Combo B (disabled – kept for reference)
# =========================
...
def decide_combo_B(symbol: str, htf_map: Dict, itf_setup: Dict, ltf_snapshot: Dict,
                   size_usdt: float, sl_mult: float, tp_mult: float) -> Optional[Decision]:
    return None  # disabled per request
...
def decide_combo_C1(symbol: str, htf_map: Dict, itf_setup: Optional[Dict], ltf_snapshot: Dict,
    ...
    line_y = _line_value(line, len(swings))  # project to "current" index
...
def decide(symbol: str, combo: str, htf_map: Dict, itf_setup: Optional[Dict], ltf_snapshot: Dict,
           size_usdt: float, sl_mult: float, tp_mult: float) -> Optional[Decision]:
    """
    combo codes:
      A    -> Combo A
      C1   -> Trendline Break + EMA9 Pullback
